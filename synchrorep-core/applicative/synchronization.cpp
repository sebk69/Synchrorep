...
GMutex				*synchronization :: ask_mutex;


// exchange between synchronize process and progession window
synchronize_exchanges :: synchronize_exchanges()
{
...
void
synchronize_exchanges :: init(char *command_line_parameter)
{
...
{
	this->exchanges = new synchronize_exchanges*[150];
...
synchronization :: ~synchronization()
{
	...
	delete this->syncs;
	g_mutex_free(synchronization :: ask_mutex);
}

void
synchronization :: new_folder(char *command_line_p)
{
	// insert synchronization at the end of then list
	int					i = 0;
	while(this->exchanges[i] != NULL)
		i++;
	this->exchanges[i] = new synchronize_exchanges;
...
{
	this->progress = g_thread_create(&this->synchronize_window, (gpointer)this->exchanges, true, NULL);
	synchronization :: ask_mutex = g_mutex_new();
...
gpointer
synchronization :: synchronize(gpointer pexchanges)
{
	synchronize_exchanges		*exchanges = (synchronize_exchanges*)pexchanges;
	...
	if(!configuration.ready_to_start())
	{
		...
		while(!g_mutex_trylock(synchronization :: ask_mutex));
		exchanges->asking_info->asking(gettext("One of the synchronization folder is missing. (disk not mounted)"));
		g_mutex_unlock(synchronization :: ask_mutex);
		exchanges->sync_step.set(_terminated);
		return NULL;
	}
	...
	while((from_not_at_the_end || to_not_at_the_end) && exchanges->sync_step.get() != _cancel)
	{
		...
		switch(compare_result)
		{
			case _from_modified:
				...
				if(binomial->copy(_binomial_from, true))
				{
					// save database dates
					to_file->refresh();
...
gpointer
synchronization :: synchronize_window(gpointer pexchanges)
{
	synchronize_exchanges	**exchanges = (synchronize_exchanges**)pexchanges;
	...
	g_signal_connect(G_OBJECT(progress_window), "destroy", (GCallback)synchronization :: synchronize_window_close, exchanges);
	g_signal_connect(G_OBJECT(cancel_btn), "clicked", (GCallback)synchronization :: synchronize_cancel, exchanges);

	g_timeout_add(50, synchronization :: synchronize_window_hook, exchanges);
...
void
synchronization :: synchronize_cancel(GtkWidget *window_widnget, gpointer data)
{
	synchronize_exchanges			**exchanges = (synchronize_exchanges**)data;
	int								i = 0;
...
void
synchronization :: synchronize_window_close(GtkWidget *window_widnget, gpointer data)
{
	synchronize_exchanges			**exchanges = (synchronize_exchanges**)data;
	int								i = 0;
	while(exchanges[i] != NULL)
...
gboolean
synchronization :: synchronize_window_hook(gpointer data)
{
	synchronize_exchanges			**exchanges = (synchronize_exchanges**)data;
	int								i = 0;
	int								terminated = 0;
